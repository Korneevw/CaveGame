...
{
    public static bool PlaceBlock(Blocks.BlockID block, Point position) 
    {
        try
        {
        ...
        { 
            Debug.Log("Can't build on edges of the world or world height.");
            return false;
        }
    }
    public static bool PlaceBlock(Block b)
    {
        try
        {
    ...
    public static Block DestroyBlock(Point position)
     {
        try
        {
            Block destroyedBlock = World.BlockData[position.X, position.Y];
            Object.Destroy(World.BlockData[position.X, position.Y].GameObject);
        ...
        { 
            Debug.Log("Can't destroy on edges of the world or world height.");
            return null;
        }
    }
    public static bool AreNeighboursSolid(Point position)
    {
        bool leftIsSolid, rightIsSolid, topIsSolid, bottomIsSolid;
        leftIsSolid = rightIsSolid = topIsSolid = bottomIsSolid = false;
    ...
    public static Block FindSurfaceBlock(int x)
    {
        for (int y = 0; y <= World.BlockData.GetUpperBound(1); y++)
        {
            try
            {
                if (World.BlockData[x, y + 1].ID == Blocks.BlockID.Air)
                {

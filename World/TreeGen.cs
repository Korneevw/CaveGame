...
public class TreeGen : MonoBehaviour // Generates trees
{
    ...
    public static void CreateProceduralTree(int x, int trunkLengthMin, int trunkLengthMax, int subTrunkLengthMin, int subTrunkLengthMax, int spaceToTheGround, int leavesWidth, int leavesHeight)
    {
        ...
        List<OakLog> trunk = new List<OakLog>();

        Block surfaceBlock = BlockManager.FindSurfaceBlock(x);
        Block startingBlock = World.BlockData[surfaceBlock.Position.X, surfaceBlock.Position.Y + 1]; // Block to build tree from
    ...
    {
        Point position = BlockManager.FindSurfaceBlock(x).Position;
        int height = Random.Range(minHeight, maxHeight + 1);
        for (int logY = 0; logY < height; logY++) // Vertical logs
        {
            BlockManager.PlaceBlock(Blocks.BlockID.OakLog, new Point(position.X, position.Y + logY));
        }
        for (int leafX = 0; leafX < 3; leafX++) // 3 Horizontal leaves
        {
            BlockManager.PlaceBlock(Blocks.BlockID.OakLeaf, new Point(position.X - 1 + leafX, position.Y + height));
        }
        BlockManager.PlaceBlock(Blocks.BlockID.OakLeaf, new Point(position.X, position.Y + height + 1)); // 1 Leaf on top
    }
}
